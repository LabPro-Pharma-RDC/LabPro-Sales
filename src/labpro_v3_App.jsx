...
const loadReports = () => { try{return JSON.parse(localStorage.getItem("lp3_reports")||"[]");}catch{return[];} };
const saveReports = r  => localStorage.setItem("lp3_reports",JSON.stringify(r));
const loadUser    = () => { try{return JSON.parse(localStorage.getItem("lp3_user")||"null");}catch{return null;} };
const saveUser    = u  => localStorage.setItem("lp3_user",JSON.stringify(u));
const clearUser   = () => localStorage.removeItem("lp3_user");

// ═══════════════════════════════════════════════════════
// 📊  EXPORT EXCEL — HTML bien mis en forme
// ═══════════════════════════════════════════════════════
const exportExcel = (rapport) => {
  const altRow = i => i%2===0?"#F0F9FF":"#FFFFFF";
  const lignesHTML = rapport.lignes.map((l,i)=>`
    <tr>
      <td style="text-align:center;padding:7px 6px;border:1px solid #BAE6FD;background:${altRow(i)};color:#0369A1;font-weight:bold">${i+1}</td>
      <td style="padding:7px 8px;border:1px solid #BAE6FD;background:${altRow(i)};font-family:Courier New,monospace;font-weight:bold;color:#075985">${l.code}</td>
      <td style="padding:7px 8px;border:1px solid #BAE6FD;background:${altRow(i)}">${l.item}</td>
      <td style="text-align:center;padding:7px 6px;border:1px solid #BAE6FD;background:${altRow(i)};font-weight:bold">${l.qte}</td>
      <td style="text-align:right;padding:7px 10px;border:1px solid #BAE6FD;background:${altRow(i)};color:#059669;font-weight:bold">$&nbsp;${fmt(l.montant)}</td>
      <td style="padding:7px 8px;border:1px solid #BAE6FD;background:${altRow(i)};font-weight:600">${l.client}</td>
      <td style="padding:7px 8px;border:1px solid #BAE6FD;background:${altRow(i)};color:#475569">${l.adresse||"—"}</td>
      <td style="padding:7px 8px;border:1px solid #BAE6FD;background:${altRow(i)};color:#475569">${l.tel||"—"}</td>
      <td style="padding:7px 6px;border:1px solid #BAE6FD;background:${altRow(i)};font-size:9pt;color:#0369A1">${l.lat&&l.lng?`${l.lat}, ${l.lng}`:"—"}</td>
    </tr>`).join("");
...
<table>
<tr><td colspan="9" style="background:#075985;color:#FFFFFF;font-size:17pt;font-weight:bold;text-align:center;padding:14px 10px;border:2px solid #0369A1;letter-spacing:0.5px">
  &#128138; RAPPORT DES VENTES JOURNALI&#200;RES
</td></tr>
<tr><td colspan="9" style="background:#0EA5E9;color:#FFFFFF;font-size:11pt;text-align:center;padding:6px;border:1px solid #0369A1;letter-spacing:1px">
  LabPro Pharma RDC Sarl
</td></tr>
<tr><td colspan="9" style="height:10px;background:#E0F2FE"></td></tr>
<tr>
  <td colspan="3" style="background:#E0F2FE;padding:9px 12px;border:1px solid #BAE6FD;font-size:11pt"><b style="color:#075985">D&#233;l&#233;gu&#233;(e) :</b> ${rapport.prenom} ${rapport.nom}</td>
  <td colspan="2" style="background:#E0F2FE;padding:9px 12px;border:1px solid #BAE6FD;font-size:11pt"><b style="color:#075985">N&#176; Agent :</b> ${rapport.num}</td>
  <td colspan="4" style="background:#E0F2FE;padding:9px 12px;border:1px solid #BAE6FD;font-size:11pt"><b style="color:#075985">Date :</b> ${rapport.date}</td>
</tr>
<tr>
  <td colspan="9" style="background:#E0F2FE;padding:9px 12px;border:1px solid #BAE6FD;font-size:11pt">
    <b style="color:#075985">Secteurs visit&#233;s :</b> ${rapport.secteurs}
  </td>
</tr>
<tr><td colspan="9" style="height:8px"></td></tr>
<tr style="background:#0369A1;color:#FFFFFF;font-size:10pt;font-weight:bold">
  <td style="padding:9px 6px;border:1px solid #0284C7;text-align:center">N&#176;</td>
  <td style="padding:9px 8px;border:1px solid #0284C7">CODE</td>
  <td style="padding:9px 8px;border:1px solid #0284C7">D&#201;SIGNATION DU PRODUIT</td>
  <td style="padding:9px 6px;border:1px solid #0284C7;text-align:center">QT&#201;</td>
  ...
  <td style="padding:9px 8px;border:1px solid #0284C7">T&#201;L&#201;PHONE</td>
...
<tr style="background:#075985;color:#FFFFFF;font-size:12pt;font-weight:bold">
  <td colspan="4" style="padding:11px 12px;border:2px solid #0369A1;text-align:right">TOTAL CHIFFRE D'AFFAIRES JOURNALIER :</td>
  <td style="padding:11px 12px;border:2px solid #0369A1;text-align:right;color:#BAE6FD;font-size:14pt">$ ${fmt(rapport.totalCA)}</td>
  <td colspan="4" style="padding:11px 12px;border:2px solid #0369A1;font-size:10pt">${rapport.nbClients} client(s) visit&#233;(s) &nbsp;|&nbsp; ${rapport.lignes.length} ligne(s) de vente</td>
</tr>
<tr><td colspan="9" style="height:12px"></td></tr>
<tr>
  <td colspan="5" style="background:#F0F9FF;padding:8px 12px;border:1px solid #BAE6FD;font-size:9pt;color:#64748B;font-style:italic">
    G&#233;n&#233;r&#233; automatiquement &#8212; LabPro Pharma RDC Sales System &#8212; ${new Date().toLocaleString("fr-FR")}
  </td>
  <td colspan="4" style="background:#F0F9FF;padding:8px 12px;border:1px solid #BAE6FD;text-align:right;font-size:10pt">
    <b>Signature du D&#233;l&#233;gu&#233;(e) :</b>&nbsp;&nbsp;_______________________
  </td>
</tr>
</table></body></html>`;
...
const exportExcelCompil = (reports, titre) => {
  const rows = reports.map((r,i)=>`
    <tr>
      <td style="text-align:center;padding:6px;border:1px solid #BAE6FD;background:${i%2===0?"#F0F9FF":"#FFF"}">${r.num||"—"}</td>
      <td style="padding:6px 10px;border:1px solid #BAE6FD;background:${i%2===0?"#F0F9FF":"#FFF"};font-weight:bold">${r.prenom} ${r.nom}</td>
      <td style="padding:6px;border:1px solid #BAE6FD;background:${i%2===0?"#F0F9FF":"#FFF"}">${r.date}</td>
      <td style="text-align:center;padding:6px;border:1px solid #BAE6FD;background:${i%2===0?"#F0F9FF":"#FFF"}">${r.lignes?.length||0}</td>
      <td style="text-align:center;padding:6px;border:1px solid #BAE6FD;background:${i%2===0?"#F0F9FF":"#FFF"}">${r.nbClients||0}</td>
      <td style="text-align:right;padding:6px 10px;border:1px solid #BAE6FD;background:${i%2===0?"#F0F9FF":"#FFF"};color:#059669;font-weight:bold">$ ${fmt(r.totalCA||0)}</td>
    </tr>`).join("");
...
<tr><td colspan="6" style="background:#075985;color:#FFF;font-size:16pt;font-weight:bold;text-align:center;padding:12px;border:2px solid #0369A1">
  COMPILATION — ${titre} — LabPro Pharma RDC Sarl
</td></tr>
<tr style="background:#0369A1;color:#FFF;font-weight:bold">
  <td style="padding:8px;border:1px solid #0284C7;text-align:center">N&#176;</td>
  <td style="padding:8px;border:1px solid #0284C7">D&#201;L&#201;GU&#201;(E)</td>
...
${rows}
<tr style="background:#075985;color:#FFF;font-weight:bold;font-size:12pt">
  <td colspan="5" style="padding:10px 12px;border:2px solid #0369A1;text-align:right">TOTAL G&#201;N&#201;RAL :</td>
...
// ═══════════════════════════════════════════════════════
// 🗺️  SUPABASE GPS & PLAINTES
// ═══════════════════════════════════════════════════════
const saveGPSPoint = async (user, point) => {
  await db.from("gps_points").insert({
    delegue_id: user.id, nom: user.nom, prenom: user.prenom, num: user.num,
...
const savePlainte = async (user, plainte) => {
  const {data,error} = await db.from("plaintes").insert({
    delegue_id: user.id, nom: user.nom, prenom: user.prenom, num: user.num,
    type_plainte: plainte.type, description: plainte.description,
    urgence: plainte.urgence, lat: plainte.lat, lng: plainte.lng,
    statut: "nouveau", created_at: nowTS(), session_date: todayISO(),
  }).select().single();
  return {data,error};
};
  ...
  return (
    <div style={{background:C.white,border:`1px solid ${C.border}`,borderRadius:14,padding:"18px 20px",position:"relative",overflow:"hidden",transition:"box-shadow 0.15s"}}
      ...
      <div style={{marginLeft:8}}>
        ...
        <div style={{fontSize:24,fontWeight:800,color:C.text,letterSpacing:-0.5,marginTop:6,lineHeight:1}}>{value}</div>
...
function Btn({children,onClick,disabled,variant="primary",icon,sm}) {
  const styles={
    primary:{background:disabled?C.light:C.sky,color:"#FFF",boxShadow:disabled?"none":"0 2px 10px rgba(14,165,233,0.3)"},
    secondary:{background:C.white,color:C.muted,border:`1px solid ${C.border}`},
    danger:{background:C.redL,color:C.red,border:`1px solid #FCA5A5`},
    success:{background:C.greenL,color:C.green,border:`1px solid #A7F3D0`},
  };
  const s=styles[variant]||styles.primary;
  return (
    <button onClick={onClick} disabled={disabled}
      style={{...s,border:s.border||"none",borderRadius:9,padding:sm?"6px 12px":"10px 18px",fontSize:sm?11:13,fontWeight:700,cursor:disabled?"not-allowed":"pointer",fontFamily:"inherit",display:"inline-flex",alignItems:"center",gap:6,transition:"all 0.15s",whiteSpace:"nowrap"}}>
...
// ═══════════════════════════════════════════════════════
// 🔐  PAGE CONNEXION — sécurisée (pas de mot de passe visible)
// ═══════════════════════════════════════════════════════
function AuthPage({onLogin}) {
  ...
  const handleLogin = async () => {
    ...
    const u=USERS.find(u=>u.email.toLowerCase()===email.toLowerCase().trim()&&u.password===pw.trim());
    if(u) onLogin(u);
    else setErr("Email ou mot de passe incorrect.");
    setLoading(false);
  };

  const inp={width:"100%",border:`1.5px solid ${C.border}`,borderRadius:10,padding:"11px 14px",fontSize:14,outline:"none",fontFamily:"inherit",boxSizing:"border-box",background:"#F8FCFF",color:C.text,transition:"border 0.15s"};
    ...
    <div style={{minHeight:"100vh",background:`linear-gradient(135deg,#0C4A6E 0%,#0369A1 50%,#0EA5E9 100%)`,display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'Inter',system-ui,sans-serif"}}>
      <div style={{width:"100%",maxWidth:460}}>
        <div style={{textAlign:"center",marginBottom:28}}>
          <div style={{display:"inline-flex",alignItems:"center",justifyContent:"center",width:70,height:70,borderRadius:20,background:"rgba(255,255,255,0.2)",backdropFilter:"blur(10px)",marginBottom:14,border:"2px solid rgba(255,255,255,0.3)"}}>
        ...
        <div style={{background:C.white,borderRadius:20,overflow:"hidden",boxShadow:"0 24px 60px rgba(0,0,0,0.3)"}}>
          <div style={{display:"flex",borderBottom:`1px solid ${C.border}`}}>
            {[["login","🔑 Connexion"],["comptes","👥 Utilisateurs"]].map(([id,lbl])=>(
              <button key={id} onClick={()=>setTab(id)} style={{flex:1,padding:"14px",border:"none",background:tab===id?C.white:"#F8FCFF",color:tab===id?C.sky:C.muted,fontWeight:tab===id?700:500,fontSize:13,cursor:"pointer",fontFamily:"inherit",borderBottom:tab===id?`2px solid ${C.sky}`:"2px solid transparent",transition:"all 0.15s"}}>{lbl}</button>
            ))}
          </div>
          ...
          <div style={{padding:28}}>
            {tab==="login"&&(
              <div>
                <div style={{marginBottom:16}}>
                  <label style={{display:"block",fontSize:11,fontWeight:700,color:C.muted,marginBottom:5,textTransform:"uppercase",letterSpacing:0.5}}>Email professionnel</label>
                  <input value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleLogin()} type="email" placeholder="prenom.nom@labpro.cd"
                    style={inp} onFocus={e=>e.target.style.border=`1.5px solid ${C.sky}`} onBlur={e=>e.target.style.border=`1.5px solid ${C.border}`}/>
                </div>
                <div style={{marginBottom:20}}>
                  ...
                  <div style={{position:"relative"}}>
                    <input value={pw} onChange={e=>setPw(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleLogin()} type={showPw?"text":"password"} placeholder="Entrez votre mot de passe"
                      style={{...inp,paddingRight:42}} onFocus={e=>e.target.style.border=`1.5px solid ${C.sky}`} onBlur={e=>e.target.style.border=`1.5px solid ${C.border}`}/>
                ...
                <button onClick={handleLogin} disabled={loading} style={{width:"100%",background:loading?"#BAE6FD":C.sky,color:"#FFF",border:"none",borderRadius:10,padding:"13px",fontSize:15,fontWeight:700,cursor:loading?"not-allowed":"pointer",fontFamily:"inherit",boxShadow:"0 4px 16px rgba(14,165,233,0.4)",transition:"all 0.15s"}}>
            ...
            {tab==="comptes"&&(
              <div>
                <div style={{background:C.skyLL,border:`1px solid ${C.border}`,borderRadius:8,padding:"10px 12px",marginBottom:12,fontSize:12,color:C.nav1}}>
                  🔒 <strong>Sécurité :</strong> Les mots de passe ne sont jamais affichés. Cliquez sur votre nom pour pré-remplir votre email uniquement — vous devrez taper votre mot de passe.
                </div>
                <div style={{maxHeight:320,overflowY:"auto",display:"flex",flexDirection:"column",gap:4}}>
                  {USERS.map(u=>(
                    <div key={u.id}
                      onClick={()=>{setEmail(u.email);setPw("");setTab("login");}}
                      style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"9px 12px",borderRadius:9,cursor:"pointer",border:`1px solid ${C.border}`,transition:"all 0.12s"}}
                      onMouseEnter={e=>{e.currentTarget.style.background=C.skyLL;e.currentTarget.style.borderColor=C.skyL;}}
                      onMouseLeave={e=>{e.currentTarget.style.background="transparent";e.currentTarget.style.borderColor=C.border;}}>
                      <div style={{display:"flex",alignItems:"center",gap:10}}>
                        <div style={{width:32,height:32,borderRadius:"50%",background:ROLE_COLOR[u.role],display:"flex",alignItems:"center",justifyContent:"center",color:"#FFF",fontSize:11,fontWeight:700,flexShrink:0}}>
                      ...
                      <Badge color={u.role==="directeur"?"navy":u.role==="superviseur"?"purple":u.role==="comptable"?"amber":"sky"}>{u.role}</Badge>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <p style={{textAlign:"center",fontSize:11,color:"rgba(255,255,255,0.3)",marginTop:14}}>LabPro Pharma RDC · v3.0 · 2025</p>
      </div>
    </div>
  );
}
...
const NAV = {
  ...
  directeur:  [{id:"dashboard",icon:"📊",lbl:"Tableau de bord"},{id:"rapports",icon:"📋",lbl:"Tous les rapports"},{id:"hebdo",icon:"📅",lbl:"Rapport hebdomadaire"},{id:"mensuel",icon:"🗓",lbl:"Rapport mensuel"},{id:"classement",icon:"🏆",lbl:"Classement délégués"},{id:"localisation",icon:"🗺️",lbl:"Localisation terrain"},{id:"plaintes",icon:"⚠️",lbl:"Signalements & Plaintes"}],
      ...
      <div style={{padding:"20px 18px 16px",borderBottom:"1px solid rgba(255,255,255,0.1)"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            ...
            <div style={{color:"rgba(255,255,255,0.4)",fontSize:10,letterSpacing:0.5}}>RDC Sarl · Sales v3</div>
          </div>
        </div>
      </div>
      <div style={{padding:"12px 14px",borderBottom:"1px solid rgba(255,255,255,0.08)"}}>
        <div style={{display:"flex",alignItems:"center",gap:9}}>
          <div style={{width:36,height:36,borderRadius:"50%",background:ROLE_COLOR[user.role],display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:800,color:"#FFF",flexShrink:0}}>
  ...
  if(done) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"60vh"}}>
      <Card style={{textAlign:"center",maxWidth:480,padding:48}}>
        ...
        <div style={{background:C.skyLL,borderRadius:12,padding:"12px 18px",marginBottom:16,display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,textAlign:"center"}}>
        ...
        <p style={{color:C.muted,fontSize:13,marginBottom:20}}>📊 Fichier Excel téléchargé · Email envoyé à la direction</p>
    ...
    <div style={{animation:"fadeIn 0.3s ease"}}>
      ...
      <Card style={{marginBottom:16}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 2fr",gap:18,alignItems:"end"}}>
          ...
          <div><div style={{fontSize:10,color:C.muted,fontWeight:700,textTransform:"uppercase",letterSpacing:0.5,marginBottom:5}}>Secteurs / Zones visités *</div>
          <input value={secteurs} onChange={e=>setSecteurs(e.target.value)} placeholder="Ex : Gombe, Lingwala, Barumbu..." style={{...inp2,padding:"9px 12px",fontSize:13}}/></div>
        </div>
      </Card>
            ...
            <tbody>
              {lignes.map((row,idx)=>(
                <tr key={row.id} style={{borderBottom:`1px solid ${C.skyLL}`}}
                  ...
                  <td style={{padding:"7px 8px",textAlign:"center",color:C.light,fontSize:12,fontWeight:600}}>{idx+1}</td>
                  <td style={{padding:"7px 6px",minWidth:105}}>
                    <select value={row.code} onChange={e=>updRow(row.id,"code",e.target.value)} style={{...inp2,minWidth:100,cursor:"pointer"}}>
                  ...
                  <td style={{padding:"7px 6px",width:115}}>
                    <div style={{fontSize:13,fontWeight:700,color:row.montant>0?C.green:C.light,background:row.montant>0?C.greenL:C.bg,borderRadius:7,padding:"6px 10px",textAlign:"right",minWidth:95}}>
                  ...
                  <td style={{padding:"7px 6px",minWidth:115}}>
                    {row.lat
      ...
      <div style={{display:"flex",justifyContent:"flex-end",gap:10,paddingBottom:24}}>
        ...
        <Btn onClick={submit} disabled={saving} icon={saving?"⏳":"📤"}>{saving?"Envoi en cours...":"Soumettre et envoyer"}</Btn>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{animation:"fadeIn 0.3s ease"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14,marginBottom:20}}>
        ...
        <StatCard icon="💰" label="CA cumulé (USD)" value={`$${fmt(totalCA)}`} color={C.amber}/>
      </div>
      {!all.length
        ? <Card style={{textAlign:"center",padding:56,color:C.muted}}>Aucun rapport soumis pour l'instant.</Card>
            ...
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              ...
              <tbody>{all.map(r=>(
                <tr key={r.id} style={{borderBottom:`1px solid ${C.skyLL}`}}
                  ...
                  <td style={{padding:"11px 14px"}}><Badge>{r.lignes?.length||0} ventes</Badge></td>
...
// ═══════════════════════════════════════════════════════
// 📍  GPS TERRAIN — DÉLÉGUÉ (suivi de trajectoire)
// ═══════════════════════════════════════════════════════
function GPSTerrain({user}) {
  ...
  const startTracking=()=>{
    if(!navigator.geolocation){setStatus("⚠️ GPS non disponible sur cet appareil.");return;}
    setStatus("⏳ Acquisition du signal GPS...");
    startRef.current=Date.now();
    timer.current=setInterval(()=>setDuration(Math.floor((Date.now()-startRef.current)/1000)),1000);
    watchId.current=navigator.geolocation.watchPosition(
      pos=>{
        ...
        setStatus(`✅ Signal GPS actif · Précision : ±${pt.accuracy}m`);
  ...
  const fmt_dur=(s)=>{const m=Math.floor(s/60);const ss=s%60;return`${m}min ${ss}s`;};
  const last=trail[trail.length-1];

  return (
    <div style={{animation:"fadeIn 0.3s ease"}}>
      <PageTitle title="📍 Mon GPS Terrain" sub="Suivi de votre itinéraire en temps réel · Données transmises au Superviseur et Directeur"/>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18}}>
        {/* Controls */}
        <div>
          <Card style={{marginBottom:14}}>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
              <div style={{width:48,height:48,borderRadius:12,background:tracking?"#ECFDF5":C.skyLL,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24}}>{tracking?"🟢":"⚪"}</div>
              <div>
                <div style={{fontSize:14,fontWeight:700,color:C.text}}>{tracking?"Tracking ACTIF":"Tracking INACTIF"}</div>
                <div style={{fontSize:12,color:C.muted}}>{tracking?`Durée : ${fmt_dur(duration)}`:"Appuyez pour démarrer"}</div>
              </div>
            </div>
            ...
            <div style={{display:"flex",gap:10}}>
              {!tracking
                ? <Btn onClick={startTracking} icon="▶️">Démarrer le suivi GPS</Btn>
                : <Btn onClick={stopTracking} variant="danger" icon="⏹">Arrêter le suivi</Btn>
              }
              {trail.length>0&&!tracking&&<Btn onClick={()=>setTrail([])} variant="secondary" icon="🗑">Effacer</Btn>}
            </div>
          </Card>

          <Card>
            <h3 style={{fontSize:13,fontWeight:700,color:C.text,margin:"0 0 12px"}}>📊 Statistiques de session</h3>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              {[["📍","Points GPS",trail.length],["⏱","Durée",fmt_dur(duration)],["🎯","Précision",last?`±${last.accuracy}m`:"—"],["📅","Date",todayISO()]].map(([i,l,v])=>(
        ...
        {/* Trail log */}
        <Card>
          <h3 style={{fontSize:13,fontWeight:700,color:C.text,margin:"0 0 12px"}}>🗺️ Historique des points ({trail.length})</h3>
          {!trail.length
            ? <div style={{height:200,display:"flex",alignItems:"center",justifyContent:"center",color:C.light,flexDirection:"column",gap:8}}>
                <div style={{fontSize:36}}>📍</div>
                <div style={{fontSize:13}}>Démarrez le suivi pour voir les points</div>
              </div>
            : <div style={{maxHeight:380,overflowY:"auto",display:"flex",flexDirection:"column",gap:4}}>
                {[...trail].reverse().map((pt,i)=>(
                  <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",background:i===0?C.skyLL:C.bg,borderRadius:8,border:`1px solid ${C.border}`}}>
                    <div>
                      <div style={{fontSize:12,fontWeight:600,color:C.text}}>📍 {pt.lat}, {pt.lng}</div>
                      <div style={{fontSize:10,color:C.muted}}>Précision : ±{pt.accuracy}m · {new Date(pt.ts).toLocaleTimeString("fr-FR")}</div>
                    </div>
                    {i===0&&<span style={{background:C.green,color:"#FFF",fontSize:9,fontWeight:700,padding:"2px 8px",borderRadius:999}}>DERNIER</span>}
                  </div>
                ))}
              </div>
          }
          {last&&(
            <a href={`https://maps.google.com/maps?q=${last.lat},${last.lng}`} target="_blank" rel="noopener noreferrer"
              style={{display:"block",marginTop:12,textAlign:"center",background:C.skyLL,color:C.nav2,border:`1px solid ${C.border}`,borderRadius:8,padding:"9px",fontSize:12,fontWeight:700,textDecoration:"none"}}>
...
function PlainteForm({user}) {
  ...
  if(done) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"50vh"}}>
      <Card style={{textAlign:"center",maxWidth:440,padding:48}}>
        ...
        <p style={{color:C.muted,fontSize:13,marginBottom:20}}>Votre signalement a été transmis au Superviseur et au Directeur Commercial.</p>
  ...
  const inp={width:"100%",border:`1.5px solid ${C.border}`,borderRadius:9,padding:"10px 13px",fontSize:13,outline:"none",fontFamily:"inherit",boxSizing:"border-box",background:"#F8FCFF",color:C.text};
  const lbl={display:"block",fontSize:11,fontWeight:700,color:C.muted,marginBottom:6,textTransform:"uppercase",letterSpacing:0.5};

  return (
    <div style={{animation:"fadeIn 0.3s ease",maxWidth:680}}>
      <PageTitle title="⚠️ Signaler un problème" sub="Votre signalement sera transmis au Superviseur et au Directeur Commercial"/>
      <Card>
        <div style={{marginBottom:16}}>
        ...
        <div style={{marginBottom:16}}>
          <label style={lbl}>Niveau d'urgence</label>
          <div style={{display:"flex",gap:10}}>
            {[["normale","🟡 Normale"],["haute","🟠 Haute"],["critique","🔴 Critique"]].map(([v,lbl2])=>(
        ...
        <div style={{marginBottom:16}}>
          ...
          <textarea value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))} placeholder="Décrivez le problème en détail : quoi, où, avec qui, quand..."
            style={{...inp,height:120,resize:"vertical"}}/>
        </div>
        <div style={{marginBottom:20}}>
          <label style={lbl}>Localisation GPS (optionnel)</label>
          <div style={{display:"flex",gap:10,alignItems:"center"}}>
            {loc.lat
              ? <div style={{flex:1,background:C.greenL,border:`1px solid #A7F3D0`,borderRadius:9,padding:"9px 12px",fontSize:12,color:C.green,fontWeight:600}}>📍 {loc.lat}, {loc.lng}</div>
        ...
        <Btn onClick={submit} disabled={saving} icon={saving?"⏳":"📤"}>
...
// ═══════════════════════════════════════════════════════
// 🗺️  LOCALISATION MAP — SUPERVISEUR & DIRECTEUR
// ═══════════════════════════════════════════════════════
function LocalisationMap() {
  ...
  const loadLeaflet=(pts)=>{
    const initMap=()=>{
      ...
      Object.values(byDel).forEach((d,i)=>{
        ...
        L.circleMarker(last,{radius:8,color:col,fillColor:col,fillOpacity:1}).addTo(map)
      ...
      if(pts.length) map.fitBounds(pts.map(p=>[p.lat,p.lng]),{padding:[20,20]});
      mapInst.current=map;
    };
  ...
  const delegsList=Object.values(byDel);

  return (
    <div style={{animation:"fadeIn 0.3s ease"}}>
      <PageTitle title="🗺️ Localisation des Délégués" sub={`Terrain aujourd'hui · ${todayISO()} · Actualisation auto toutes les 30s`}
        action={<Btn onClick={loadData} variant="secondary" icon="🔄">Actualiser</Btn>}/>
      <div style={{display:"grid",gridTemplateColumns:"280px 1fr",gap:16}}>
        <div>
          <Card style={{marginBottom:12}}>
            <div style={{fontSize:11,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:0.5,marginBottom:10}}>Délégués sur le terrain</div>
            {loading&&<div style={{textAlign:"center",color:C.muted,padding:"20px 0",fontSize:13}}>⏳ Chargement...</div>}
            {!loading&&!delegsList.length&&(
              <div style={{textAlign:"center",color:C.muted,padding:"24px 0"}}>
                <div style={{fontSize:32,marginBottom:8}}>📍</div>
                <div style={{fontSize:12}}>Aucun délégué actif aujourd'hui</div>
              </div>
            )}
            {delegsList.map((d,i)=>{
              const last=d.pts[d.pts.length-1];
              return (
                <div key={d.id} onClick={()=>setSelDel(selDel===d.id?null:d.id)}
                  style={{padding:"10px 12px",borderRadius:9,marginBottom:6,cursor:"pointer",border:`2px solid ${selDel===d.id?CHART_COLS[i%CHART_COLS.length]:C.border}`,background:selDel===d.id?C.skyLL:C.bg,transition:"all 0.12s"}}>
                  ...
                  {selDel===d.id&&last&&(
                    <div style={{marginTop:8,fontSize:10,color:C.muted,lineHeight:1.6,background:C.white,borderRadius:6,padding:"6px 8px"}}>
                      <div>📍 {last.lat}, {last.lng}</div>
                      <div>🕐 {new Date(last.created_at).toLocaleTimeString("fr-FR")}</div>
                      <div>🎯 ±{last.accuracy}m</div>
                      <a href={`https://maps.google.com/maps?q=${last.lat},${last.lng}`} target="_blank" rel="noopener noreferrer"
                        style={{display:"block",marginTop:5,color:C.sky,fontWeight:600,textDecoration:"none"}}>📌 Voir sur Maps →</a>
                    </div>
                  )}
                </div>
              );
          ...
          <Card>
            <div style={{fontSize:11,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:0.5,marginBottom:8}}>Légende</div>
            {delegsList.map((d,i)=>(
              <div key={d.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:5}}>
                <div style={{width:16,height:3,background:CHART_COLS[i%CHART_COLS.length],borderRadius:2}}/>
                <span style={{fontSize:11,color:C.muted}}>{d.prenom} {d.nom.split(" ")[0]}</span>
              </div>
            ))}
          </Card>
        </div>
        <div ref={mapRef} style={{height:"calc(100vh - 200px)",minHeight:460,borderRadius:14,border:`1px solid ${C.border}`,background:C.bg,position:"relative"}}>
...
// ═══════════════════════════════════════════════════════
// ⚠️  PLAINTES VIEW — SUPERVISEUR & DIRECTEUR
// ═══════════════════════════════════════════════════════
function PlaintesView() {
  ...
  useEffect(()=>{
    loadPlaintes().then(d=>{setPlaintes(d);setLoading(false);});
  },[]);
  ...
  return (
    <div style={{animation:"fadeIn 0.3s ease"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:18}}>
        ...
        <StatCard icon="🚨" label="Critiques" value={plaintes.filter(p=>p.urgence==="critique").length} color="#DC2626"/>
      </div>

      <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
        {[["all","Tous"],["nouveau","Nouveaux"],["en_cours","En cours"],["traite","Traités"]].map(([v,l])=>(
          <button key={v} onClick={()=>setFilter(v)}
            style={{padding:"7px 16px",borderRadius:999,border:`1.5px solid ${filter===v?C.sky:C.border}`,background:filter===v?C.sky:C.white,color:filter===v?"#FFF":C.muted,fontWeight:filter===v?700:500,fontSize:12,cursor:"pointer",fontFamily:"inherit",transition:"all 0.12s"}}>{l}</button>
        ))}
      </div>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:12}}>
                <div style={{flex:1}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6,flexWrap:"wrap"}}>
                    ...
                    <span style={{background:urgColor[p.urgence]+"22",color:urgColor[p.urgence],fontSize:10,fontWeight:700,padding:"2px 8px",borderRadius:999}}>
                  ...
                  <div style={{display:"flex",gap:12,marginTop:6,flexWrap:"wrap"}}>
                    ...
                    {p.lat&&p.lng&&(
                      <a href={`https://maps.google.com/maps?q=${p.lat},${p.lng}`} target="_blank" rel="noopener noreferrer" style={{fontSize:11,color:C.sky,fontWeight:600,textDecoration:"none"}}>📍 Voir position</a>
                    )}
                  </div>
                </div>
...
function Dashboard({user}) {
  ...
  const deleg=USERS.filter(u=>u.role==="delegue");
  ...
  return (
    <div style={{animation:"fadeIn 0.3s ease"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:18}}>
        <StatCard icon="📊" label="Rapports aujourd'hui" value={todayR.length} sub={`${[...new Set(todayR.map(r=>r.delegueId))].length}/${deleg.length} délégués`} color={C.sky}/>
      ...
      <div style={{display:"grid",gridTemplateColumns:"3fr 2fr",gap:16,marginBottom:16}}>
        ...
        <Card>
          ...
          {pieData.length
            ? <ResponsiveContainer width="100%" height={190}>
                <PieChart><Pie data={pieData} cx="50%" cy="50%" innerRadius={48} outerRadius={82} dataKey="value">
                  {pieData.map((_,i)=><Cell key={i} fill={CHART_COLS[i%CHART_COLS.length]}/>)}
                </Pie><Tooltip contentStyle={{borderRadius:10,border:`1px solid ${C.border}`,fontSize:11}}/><Legend wrapperStyle={{fontSize:10}}/></PieChart>
              </ResponsiveContainer>
      ...
      <Card>
        ...
        {barData.length
          ? <ResponsiveContainer width="100%" height={195}>
              <BarChart data={barData} barSize={34}><CartesianGrid strokeDasharray="3 3" stroke={C.skyLL}/>
                <XAxis dataKey="name" tick={{fontSize:11,fill:C.muted}} axisLine={false} tickLine={false}/>
                <YAxis tick={{fontSize:10,fill:C.muted}} axisLine={false} tickLine={false}/>
                <Tooltip formatter={v=>`$${fmt(v)}`} contentStyle={{borderRadius:10,border:`1px solid ${C.border}`,fontSize:11}}/>
                <Bar dataKey="ca" fill={C.sky} radius={[6,6,0,0]} label={{position:"top",fontSize:10,fill:C.muted,formatter:v=>v>0?`$${v}`:""}}/>
              </BarChart>
            </ResponsiveContainer>
...
function TousLesRapports() {
  const [search,setSearch]=useState(""); const [dateF,setDateF]=useState(""); const [delF,setDelF]=useState(""); const [exp,setExp]=useState(null);
  const all=loadReports();
  const filtered=all.filter(r=>{
    const q=search.toLowerCase();
    return(!search||`${r.prenom} ${r.nom} ${r.secteurs}`.toLowerCase().includes(q)||r.lignes?.some(l=>l.client.toLowerCase().includes(q)))&&(!dateF||r.date===dateF)&&(!delF||r.delegueId===delF);
  ...
  return (
    <div style={{animation:"fadeIn 0.3s ease"}}>
      <PageTitle title="📋 Tous les rapports" sub={`${filtered.length} rapport(s)`}
        action={<Btn onClick={()=>exportExcelCompil(filtered,"Tous Rapports")} icon="⬇">Exporter Excel</Btn>}/>
      <div style={{display:"grid",gridTemplateColumns:"1fr auto auto auto",gap:10,marginBottom:16,alignItems:"center"}}>
        ...
        <select value={delF} onChange={e=>setDelF(e.target.value)} style={{border:`1.5px solid ${C.border}`,borderRadius:10,padding:"9px 12px",fontSize:13,outline:"none",fontFamily:"inherit",background:"#F8FCFF"}}>
        ...
        {(search||dateF||delF)&&<Btn onClick={()=>{setSearch("");setDateF("");setDelF("");}} variant="danger" sm icon="✕">Effacer</Btn>}
      </div>
      {!filtered.length
        ? <Card style={{textAlign:"center",padding:48,color:C.muted}}>Aucun rapport trouvé.</Card>
        : <Card p="0" style={{overflow:"hidden"}}>
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead><tr style={{background:C.bg}}>
                ...
                {filtered.map(r=>(
                  ...
                  <tr key={r.id} style={{borderBottom:`1px solid ${C.skyLL}`,cursor:"pointer"}}
                    ...
                    <td style={{padding:"10px 13px"}}>
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        <div style={{width:28,height:28,borderRadius:"50%",background:C.sky,display:"flex",alignItems:"center",justifyContent:"center",color:"#FFF",fontSize:10,fontWeight:700,flexShrink:0}}>{(r.prenom[0]+r.nom[0]).toUpperCase()}</div>
...
function Classement() {
  ...
  return (
    <div style={{animation:"fadeIn 0.3s ease"}}>
      ...
      {!ranking.length
        ? <Card style={{textAlign:"center",padding:56,color:C.muted}}>Aucune donnée ce mois.</Card>
        : <>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:14,marginBottom:20}}>
            {ranking.slice(0,3).map((d,i)=>(
              <Card key={i} style={{textAlign:"center",border:`2px solid ${mColors[i]||C.border}`,boxShadow:i===0?"0 8px 28px rgba(217,119,6,0.15)":"none"}}>
                ...
                <div style={{width:48,height:48,borderRadius:"50%",background:mColors[i]||C.sky,display:"flex",alignItems:"center",justifyContent:"center",fontSize:17,fontWeight:800,color:"#FFF",margin:"0 auto 10px"}}>
              ...
              <tbody>{ranking.map((d,i)=>(
                <tr key={d.id} style={{borderBottom:`1px solid ${C.skyLL}`,background:i===0?"#FFFBEB":"transparent"}}
                  ...
                  <td style={{padding:"12px 13px"}}>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      <div style={{width:32,height:32,borderRadius:"50%",background:mColors[i]||C.sky,display:"flex",alignItems:"center",justifyContent:"center",color:"#FFF",fontSize:11,fontWeight:700}}>
                        {(d.prenom[0]+d.nom[0]).toUpperCase()}
                      </div>
                      <div>
                        <div style={{fontSize:12,fontWeight:700,color:C.text}}>{d.prenom} {d.nom}</div>
                  ...
                  <td style={{padding:"12px 13px",minWidth:120}}>
                    <div style={{display:"flex",alignItems:"center",gap:7}}>
                      <div style={{background:C.bg,borderRadius:4,height:6,flex:1}}>
                        <div style={{background:i===0?C.amber:C.sky,borderRadius:4,height:6,width:`${Math.min(100,Math.round((d.jours/22)*100))}%`}}/>
                      </div>
  ...
  return (
    <div style={{animation:"fadeIn 0.3s ease"}}>
      <PageTitle title={type==="hebdo"?"📅 Rapport Hebdomadaire":"🗓 Rapport Mensuel"} sub={titre}
        action={<Btn onClick={()=>exportExcelCompil(filtered,titre)} icon="⬇">Exporter Excel</Btn>}/>
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:18}}>
        <StatCard icon="📊" label="Rapports soumis" value={filtered.length} color={C.sky}/>
        <StatCard icon="👥" label="Délégués actifs" value={rows.length} sub={`/ ${USERS.filter(u=>u.role==="delegue").length}`} color={C.nav2}/>
        ...
        <Card style={{marginBottom:16}}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={chartData} barSize={36}><CartesianGrid strokeDasharray="3 3" stroke={C.skyLL}/>
              <XAxis dataKey="name" tick={{fontSize:11,fill:C.muted}} axisLine={false} tickLine={false}/>
              <YAxis tick={{fontSize:10,fill:C.muted}} axisLine={false} tickLine={false}/>
              <Tooltip formatter={(v,n)=>[n==="ca"?`$${fmt(v)}`:v,n==="ca"?"CA USD":"Ventes"]} contentStyle={{borderRadius:10,border:`1px solid ${C.border}`,fontSize:11}}/>
              <Legend wrapperStyle={{fontSize:11}}/>
              <Bar dataKey="ca" name="CA (USD)" fill={C.sky} radius={[5,5,0,0]}/>
              <Bar dataKey="ventes" name="Nb ventes" fill={C.green} radius={[5,5,0,0]}/>
            </BarChart>
          </ResponsiveContainer>
        </Card>
      )}
      <Card p="0" style={{overflow:"hidden"}}>
        <div style={{padding:"12px 16px",background:C.nav1}}><h3 style={{color:"#FFF",fontSize:13,fontWeight:700,margin:0}}>Récapitulatif par délégué</h3></div>
        {!rows.length
          ? <div style={{padding:40,textAlign:"center",color:C.muted}}>Aucune donnée pour cette période.</div>
          : <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead><tr style={{background:C.bg}}>
                ...
                {rows.map((r,i)=>{
                  ...
                  return (
                    <tr key={r.id} style={{borderBottom:`1px solid ${C.skyLL}`}}
                      ...
                      <td style={{padding:"11px 13px",minWidth:110}}>
                        <div style={{display:"flex",alignItems:"center",gap:7}}>
                          <div style={{background:C.bg,borderRadius:4,height:6,flex:1}}><div style={{background:C.sky,borderRadius:4,height:6,width:`${pct}%`}}/></div>
                          <span style={{fontSize:11,fontWeight:700,color:C.muted,minWidth:26}}>{pct}%</span>
                        </div>
                      </td>
                    </tr>
                  );
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    s.textContent=`@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap');*{box-sizing:border-box;margin:0;padding:0;}body{font-family:'Inter',system-ui,sans-serif;background:${C.bg};-webkit-font-smoothing:antialiased;}::-webkit-scrollbar{width:5px;height:5px}::-webkit-scrollbar-track{background:${C.bg}}::-webkit-scrollbar-thumb{background:${C.skyL};border-radius:3px}@keyframes fadeIn{from{opacity:0;transform:translateY(5px)}to{opacity:1;transform:none}}`;
    document.head.appendChild(s);
    const u=loadUser();if(u){setUser(u);setView(defaultView(u.role));}
    setBooted(true);
    return()=>document.head.removeChild(s);
  },[]);

  const onLogin=u=>{setUser(u);saveUser(u);setView(defaultView(u.role));};
  const onLogout=()=>{setUser(null);clearUser();setView("");};

  if(!booted) return <div style={{minHeight:"100vh",background:`linear-gradient(135deg,${C.nav1},${C.sky})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:48}}>💊</div>;
  ...
  return (
    <div style={{display:"flex",minHeight:"100vh"}}>
      ...
      <main style={{flex:1,marginLeft:238,padding:"26px 30px",minHeight:"100vh",overflowX:"hidden",background:C.bg}}>
        {renderView()}
      </main>
    </div>
  );
